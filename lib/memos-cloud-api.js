...
export function extractResultData(result) {
  ...
  const rawData = result.data ?? result.data?.data ?? result.data?.result ?? null;
  if (!rawData) return null;

  // Cloud API format check
  if (Array.isArray(rawData.memory_detail_list) || Array.isArray(rawData.preference_detail_list)) {
    return rawData;
  }

  // Open Source API format check
  const openData = Array.isArray(rawData) ? rawData[0] : rawData;
  if (openData && (openData.text_mem || openData.fact_mem || openData.pref_mem || openData.tool_mem || openData.skill_mem)) {
    const normalizeMemories = (memGroup) => {
      if (!Array.isArray(memGroup)) return [];
      let allMems = [];
      for (const group of memGroup) {
        if (Array.isArray(group.memories)) {
          allMems = allMems.concat(group.memories);
        }
      }
      return allMems.map((m) => {
        ...
        return {
          id: m.id || meta.id,
          memory_key: meta.key || m.memory,
          memory_value: m.memory || meta.memory,
          relativity: meta.relativity ?? m.relativity,
          preference: meta.preference || m.memory,
          preference_type: meta.preference_type,
          tool_value: m.memory || meta.memory,
          create_time: meta.created_at ? new Date(meta.created_at).getTime() : Date.now(),
          update_time: meta.updated_at ? new Date(meta.updated_at).getTime() : Date.now(),
        };
      });
    };

    return {
      memory_detail_list: normalizeMemories(openData.text_mem || openData.fact_mem),
      preference_detail_list: normalizeMemories(openData.pref_mem),
      tool_memory_detail_list: normalizeMemories(openData.tool_mem),
      skill_detail_list: normalizeMemories(openData.skill_mem),
      preference_note: openData.pref_note || openData.pref_string || "",
    };
  }
...
export function buildConfig(pluginConfig = {}) {
  ...
  const apiType = cfg.apiType || loadEnvVar("MEMOS_API_TYPE") || (baseUrl.includes("memos.memtensor.cn") ? "cloud" : "open_source");
...
export async function callApi({ baseUrl, apiType, apiKey, timeoutMs = 5000, retries = 1 }, path, body) {
  const isCloud = apiType === "cloud";
  ...
  if (isCloud) {
    ...
    // For open source, it doesn't require Authorization but if provided, send it
...
export async function searchMemory(cfg, payload) {
  const path = cfg.apiType === "cloud" ? "/search/memory" : "/product/search";
  return callApi(cfg, path, sanitizeSearchPayload(payload));
}
...
export async function addMessage(cfg, payload) {
  ...
  const path = cfg.apiType === "cloud" ? "/add/message" : "/product/add";
  return callApi(cfg, path, finalPayload);
}
