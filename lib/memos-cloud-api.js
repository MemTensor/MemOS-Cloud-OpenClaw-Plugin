...
export function stripOpenClawInjectedPrefix(text) {
  ...
  // Use indexOf (first occurrence) to find USER_QUERY_MARKER, not lastIndexOf.
  // When prependContext is added to prompt, the marker appears at the boundary
  // between prepended content and original user query. indexOf finds the FIRST
  // (original) marker correctly, while lastIndexOf would find the LAST marker
  // in the prepended content and incorrectly strip the actual user query.
  const markerIndex = cleanedText.indexOf(USER_QUERY_MARKER);
  // Extract user query from BEFORE the marker (recall result is after marker in output)
  const withoutRecallPrefix =
    markerIndex === -1
      ? cleanedText
      : cleanedText.slice(0, markerIndex);
