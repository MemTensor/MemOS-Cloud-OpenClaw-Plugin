...
import { MemosClient, MemosClientError } from '../src/memos-client.ts'
import type { MemosAddRequest, MemosSearchRequest } from '../src/types.ts'
...
describe('MemosClient retry policy', () => {
  ...
  it('does not retry add requests by default', async () => {
  ...
  it('uses an adapter-configured add retry count', async () => {
    const fetch = vi.fn<typeof globalThis.fetch>()
      .mockImplementationOnce(async () => jsonResponse({ message: 'temporary' }, 503))
      .mockImplementationOnce(async () => successAdd())
    const client = createClient(fetch, { addRetries: 1 })

    await expect(client.add(addRequest)).resolves.toMatchObject({ success: true })
    expect(fetch).toHaveBeenCalledTimes(2)
  })
