...
Run these commands from the monorepo repository root. `prepare:openclaw` installs workspace dependencies and builds the shared core into this package; it does not install the plugin or restart OpenClaw.
...
This installs a copy under OpenClaw's extensions directory and does not require a `plugins.load` entry. After changing the plugin source or `packages/core`, rerun preparation and reinstall the directory, then restart the gateway.
...
### 1) Recall (`before_prompt_build`; falls back to `before_agent_start` on older OpenClaw hosts)
